...
export async function getCompanyReport(
  ...
  return adminReportAdditionalCrudService.post('organization', payload) as Promise<ICompanyReport>
}

export async function getCompanyReportTable(
  payload: ICompanyReportPayload
): Promise<PaginationList<ICompanyReportTable>> {
  return adminReportAdditionalCrudService.post('organization/table', payload) as Promise<PaginationList<ICompanyReportTable>>
}
...
export async function getReportProcessDetail(
  organizationId: number,
  processId: number,
  payload: IReportDetailPayload
): Promise<IReportDetail> {
  const response = await api.post(
    `${REPORT_URL}/${organizationId}/process/${processId}`,
    payload
  );
  return response.data;
}

export async function getReportCollectionDetail(
  organizationId: number,
  collectionId: number,
  payload: IReportDetailPayload
): Promise<IReportDetail> {
  const response = await api.post(
    `${REPORT_URL}/${organizationId}/collection/${collectionId}`,
    payload
  );
  return response.data;
}
...
export async function getReportGroupDetail(
  ...
  const response = await api.post(
    `${REPORT_URL}/${organizationId}/group/${groupId}`,
    payload
  );
  return response.data;
}
