...
export default class UserStore {
	...
	public async getAllEmergencyContacts(organizationId: string): Promise<void> {
		const admins = await getUsers({
			where: {
				organizationId,
				$or: [
					{ authRole: AuthRoleNameEnum.ORG_ADMIN },
					{ authRole: AuthRoleNameEnum.MANAGER },
				],
			},
			fields: [
				'email',
