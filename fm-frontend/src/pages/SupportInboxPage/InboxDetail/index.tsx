...
import { getCreatorOfProcess, getThreadTitle, getThumbnailType } from './utils';
...
const InboxDetail = (props: IInboxDetailProps) => {
	...
	const [showChangePriorityModal, setShowChangePriorityModal] =
		useState<boolean>(false);
	...
	const [selectedPriority, setSelectedPriority] = useState<IPriority | null>(
		selectedSupportThread?.priority
			? getPriorityConfig(selectedSupportThread?.priority)
			: null
	);
	const [editPriority, setEditPriority] = useState<IPriority | null>(
		selectedPriority
	);
	...
	async function handleChangeThreadPriority(
	...
	async function handleChangePriority() {
		handleChangeThreadPriority(editPriority);
	}
	...
	return (
		...
		>
						...
						<HStack width="full" justify="space-between">
							...
							{!isTablet && (
								<HStack>
									{selectedPriority && (
										<Stack>
											<EditablePriorityBox
												...
												variant="solid"
												onPriorityChange={(priority: IPriority | null) => {
									...
									<HStack hidden={isClaimedByOther}>
										<Button
											size="sm"
											color="gray.700"
											background="white"
			...
			/>
			<ChangePriorityModal
				isOpen={showChangePriorityModal}
