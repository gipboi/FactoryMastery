...
const UserPage = () => {
  ...
  const filterOrganizationIds: string[] = params.get('organizations')
    ? params
        .get('organizations')
        ?.split(',')
        ?.map((id: string) => String(id)) ?? []
  ...
  async function fetchUserList(): Promise<void> {
    if (isNumber(pageSize) && isNumber(pageIndex) && organizationId) {
      const filterValidRoleIds: string[] =
        isAdminPage &&
        (!validRoleIds.length ||
          validRoleIds?.some(
            (role) =>
  ...
  async function handleCreateUser(values: IUserDetailForm): Promise<void> {
    try {
      ...
      const newUserData: ICreateEditUserRequest = {
        ...
        ...(values?.userType === AuthRoleNameEnum.ORG_ADMIN
          ? { organizationId: values?.organizations?.[0]?.value }
          : { organizationId }),
        firstName,
        lastName,
        authRole: values?.userType ?? AuthRoleNameEnum.SUPER_ADMIN,
      };
