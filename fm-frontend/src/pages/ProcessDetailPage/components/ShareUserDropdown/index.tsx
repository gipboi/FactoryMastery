...
import { FormControl, InputGroup, FormLabel, HStack, Text } from '@chakra-ui/react'
...
const ShareUserDropdown = (props: IShareUserDropdownProps) => {
  const { name, label, optionsData, placeholder, chooseOptionsHandler, sharedViaGroupUsers } = props

  const { control } = useFormContext()

  const CustomOption = (props: any) => {
    const { innerRef, innerProps, data } = props
    const isSharedViaGroup = sharedViaGroupUsers?.some(user => String(user?.id) === String(data?.value))
    ...
    return (
      ...
      >
        ...
        {isSharedViaGroup && (
          <Text width="max-content" fontSize="12px" fontWeight="500" fontStyle="italic">
