...
export class UserService {
	...
	async findUser(req, res, next) {
		try {
			...
			let currentUserQuery = User.find({
				...filter?.where,
			});

			if (filter?.select) {
				currentUserQuery = currentUserQuery.select(filter?.select);
			}

			if (populateOptions) {
				currentUserQuery = currentUserQuery.populate(populateOptions);
			}

			if (filter?.order) {
				currentUserQuery = currentUserQuery.sort(
					filter?.order?.reduce(
						(acc, curr) => ({
							...acc,
							[curr?.split(' ')?.[0]]: curr?.includes('ASC') ? 1 : -1,
						}),
						{}
					)
				);
